...
class Cart
{
    ...
    public function removeProduct($product){
        if($this->contents){
            if(array_key_exists($product->slug, $this->contents)){
                //Remove Qty and Price From Array
                $rPorduct = $this->contents[$product->slug];
                $this->totalQty-=$rPorduct['qty'];
                $this->totalPrice-=$rPorduct['price'];
    ...
    public function updateProduct($product,$qty){
        if($this->contents){
            if(array_key_exists($product->slug, $this->contents)){
                //Update Qty and Price From Array
                $products = $this->contents[$product->slug];
            }
        }
